<template>
  <BasicModal v-bind="$attrs" @register="registerModal" title="服务器回收站" :showOkBtn="false" width="1000px" destroyOnClose>
...
<script lang="ts" setup>
  ...
  import { recycleColumns } from './server.data';
  import { getRecycleBinList, putRecycleBin, deleteRecycleBin } from './server.api';
  ...
  async function handleRevert(record) {
    await putRecycleBin({ serverIds: record.id }, reload);
    emit('success');
  }
  /**
   * 批量还原事件
  ...
  async function handleDelete(record) {
    await deleteRecycleBin({ serverIds: record.id }, reload);
  }
  /**
   * 批量删除事件
   */
